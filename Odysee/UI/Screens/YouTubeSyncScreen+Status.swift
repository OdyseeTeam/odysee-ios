...
extension YouTubeSyncScreen {
    struct Status: View {
        var viewChannel: (_ name: String, _ claimId: String) -> Void
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let channels = model.channels {
                    Group {
                        if ViewModel.isNotEligible(channels: channels) {
                    ...
                    Group {
                        ...
                        } else if ViewModel.isNotEligible(channels: channels) {
                            Text(
                    ...
                    List(channels) { channel in
                        Group {
                            if !channel.channelClaimId.isBlank {
                                ...
                                Button {
                                    viewChannel(channel.lbryChannelName, channel.channelClaimId)
                                } label: {
                                    NavigationLink {
                                        EmptyView()
                                    } label: {

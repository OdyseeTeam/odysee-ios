...
struct LbryUri: CustomStringConvertible {
    ...
    static let regexSeparateQueryString = try! NSRegularExpression(pattern: queryStringBreaker, options: .caseInsensitive)
    ...
    static func parse(url: String, requireProto: Bool) throws -> LbryUri {
        ...
        let results = regexComponents.matches(in: cleanUrl, options: [], range: NSRange(cleanUrl.startIndex..., in: cleanUrl))
        if (results.count > 0) {
        ...
        if (includesChannel) {
            ...
            if ((channelName ?? "").count < channelNameMinLength) {
                throw LbryUriError.runtimeError(String(format: "Channel names must be at least %d character long.", channelNameMinLength))
            }
        }
        
        var primaryMod: UriModifier? = nil, secondaryMod: UriModifier? = nil
        ...
        let streamClaimId: String? = includesChannel && secondaryMod != nil ? secondaryMod?.claimId : primaryMod != nil ? primaryMod?.claimId : nil;
        let channelClaimId: String? = includesChannel && primaryMod != nil ? primaryMod?.claimId : nil;
        
        return LbryUri(path: components[2..<components.count].joined(), isChannel: isChannel, streamName: streamName, streamClaimId: streamClaimId, channelName: channelName, channelClaimId: channelClaimId, primaryClaimSequence: primaryMod?.claimSequence ?? -1, secondaryClaimSequence: secondaryMod?.claimSequence ?? -1, primaryBidPosition: primaryMod?.bidPosition ?? -1, secondaryBidPosition: secondaryMod?.bidPosition ?? -1, claimName: streamOrChannelName, claimId: primaryMod?.claimId, contentName: streamName, queryString: queryString)
    }
    
    func build(includeProto: Bool, protoDefault: String, vanity: Bool) -> String {
        var formattedChannelName: String? = nil
        if (channelName != nil) {
        ...
        var secondaryClaimName: String? = nil

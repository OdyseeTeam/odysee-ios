...
class UserAccountViewController: UIViewController {
    lazy var signInUp = {
        let rootView = SignInUpScreen(
            ...
            close: closeButtonTapped,
            model: .init(
                finish: finishWithWalletSync,
                frRequestStarted: { self.frDelegate?.requestStarted() },
                frRequestFinished: { self.frDelegate?.requestFinished(showSkip: true, showContinue: false) }
            )
        )
